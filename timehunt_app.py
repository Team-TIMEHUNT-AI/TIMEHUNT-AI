...
def perform_ai_analysis(user_query, file_data=None, file_type=None, enable_search=False):
    """
    Handles AI requests with automatic fallback and Web Search capabilities.
    """
    try:
        from google import genai
        from google.genai import types
    ...
    # 2. MODEL PRIORITY LIST (Based on your available models)
    models_to_try = [
        "gemini-2.5-flash",       # 🚀 Best: Newest & Fastest
        "gemini-2.0-flash",       # 🛡️ Stable: Standard
        "gemini-2.0-flash-lite",  # ⚡ Lite: Backup
        "gemini-flash-latest"     # 🔄 Fallback
    ]

    # 3. CONFIGURE TOOLS (Web Search)
    model_tools = []
    if enable_search:
        # Enable Google Search Grounding if requested
        try:
            model_tools = [types.Tool(google_search=types.GoogleSearch())]
        except:
            pass # Fallback if tool config fails
    ...
    if not user_content_parts:
        return "⚠️ No content received.", "System"
    ...
    for model_name in models_to_try:
        for key in api_keys:
            ...
            try:
                ...
                # History (Text Only for stability)
                history = []
                recent = st.session_state.get('chat_history', [])[-6:]
                for msg in recent:
                    if msg.get('text') and not msg.get('file_type'):
                ...
                chat = client.chats.create(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        tools=model_tools # Inject Web Search if enabled
                    )
                )
                ...
                footer = ""
                if enable_search and response.candidates[0].grounding_metadata:
                    footer = "\n\n(🔍 Sources verified via Google Search)"
                    
                return response.text + footer, "TimeHunt AI"
...
def page_ai_assistant():
    ...
    import base64
    import io
    from gtts import gTTS # Ensure gTTS is installed
    ...
    voice_map = {"Jarvis (US)": "us", "Friday (UK)": "co.uk", "Guru (Indian)": "co.in"}
    target_tld = voice_map.get(voice_style, "us")

    # --- 2. CSS FOR GEMINI CHIPS ---
    st.markdown("""
    <style>
        /* Chip/Card Style */
        .suggestion-btn {
        ...
        /* Hide Streamlit default button styles for the grid */
    ...
    def process_message(prompt, file_data=None, file_type=None, audio_bytes=None, mode="Chat"):
        ...
        with st.chat_message("assistant", avatar=ai_av):
            ...
            # MODE B: TEXT / SEARCH
                ...
                with status: render_loading("⚡ Analyzing...")
    ...
    if not st.session_state.get('chat_history'):
        ...
        # The 4 "Gemini" Cards
        if c1.button("🎨 Create Image"):
            process_message("Generate a futuristic workspace with neon lights", mode="Image Gen")
        if c2.button("🧠 Deep Analysis"):
            process_message("Analyze my current productivity schedule and find gaps.")
        if c3.button("📚 Help me learn"):
            process_message("Explain Quantum Computing in simple terms.")
        if c4.button("⚡ Boost my day"):
            process_message("Give me a motivational quote and a quick task.")

    # --- 6. CHAT HISTORY RENDERER ---
    ...
    with chat_container:
        for i, msg in enumerate(st.session_state['chat_history']):
            ...
            with st.chat_message(role, avatar=av):
                ...
                if msg.get('text'): 
                    st.write(msg['text'])
                    # AUDIO BUTTON (Fixed Logic)
                    if role == "assistant":
                        if st.button("🔊 Listen", key=f"tts_{i}"):
                            audio_file = generate_audio(msg['text'])
                            if audio_file:
    ...
    # --- 7. INPUT AREA (Buttons BELOW Text Box) ---
    with st.container():
        with st.form(key="chat_input_form", clear_on_submit=True):
            ...
            with c_plus:
                # File Upload hidden inside expader/popover logic or just a visual cue
                uploaded_file = st.file_uploader("📎", label_visibility="collapsed")
            
            with c_mode:
                 mode = st.radio("Mode", ["Chat", "Web Search", "Image Gen"], horizontal=True, label_visibility="collapsed")
                 
            with c_send:
                submitted = st.form_submit_button("Send ➤")

        # Mic is outside form to avoid auto-submit issues
        with c_mic:
